...
目标：在任意 Linux 机器上，使用仓库自带依赖导入数据并运行实验。文档不假设固定用户名、工作目录或数据目录。
...
说明：项目 README 说测试过 Java 19.0.2。OpenJDK 17 也可用于编译和运行 Neo4j 5.3.0 AdminTool。

## 2. 准备文档仓库和 GDB_VIEW 作者仓库

这份文档和辅助脚本属于 `IncGQ-experiment-docs`，不要把辅助脚本写进作者的 `GDB_Views_VLDB2025` 仓库。先按实际机器设置路径变量：

```bash
export WORK_ROOT=/path/to/workspace
export DATA_ROOT=/path/to/datasets
export DOCS_HOME="$WORK_ROOT/IncGQ-experiment-docs"
export GDB_VIEW_HOME="$WORK_ROOT/IncGQ/GDB_Views_VLDB2025"
...
```bash
export DB_HOME="$DATA_ROOT/neo4j-dbs/ldbc_sf01"
...
```text
ldbc_sf01=<absolute-path-to-ldbc_sf01-db-home>
ldbc_sf1=<absolute-path-to-ldbc_sf1-db-home>
ldbc_sf10=<absolute-path-to-ldbc_sf10-db-home>
```
...
所以至少要保证 `ldbc_sf01` 写成刚导入的 `DB_HOME` 的绝对路径。不要在 `test/config` 中写 `~`，除非程序明确支持 shell 展开。

## 7. 修复计时输出路径

作者 GitHub 仓库中的 `src/main/Main.java` 可能仍然把计时输出写到某台机器的绝对路径。先检查：

```bash
rg -n 'FileWriter\("/' src/main/Main.java
```

如果命中，应把它改成相对路径或可配置路径。执行：
...
s = p.read_text()
import re
s = re.sub(r'new FileWriter\("/[^"]*/test/time\.txt"\)', 'new FileWriter("./test/time.txt")', s)
p.write_text(s)
PY
```
...
转换脚本应能把 LDBC 风格表头转换成 header/data 分离格式。例如关系文件会生成：
...
最小导入成功时会输出 `IMPORT DONE`。

编译验证命令：

```bash
mkdir -p "$SCRATCH_DIR/incgq-build-classes"

javac -d "$SCRATCH_DIR/incgq-build-classes" \
  -sourcepath src \
  -cp ".:lib2/apoc-5.3.0-core.jar:./lib2/*" \
  src/main/Main.java src/main/Neo4jDriverConnector.java
```
...
因为 `import/person_0_0.csv` 是相对执行导入命令时的工作目录解析的。
...
### 12.6 `FileNotFoundException` 指向某个绝对路径
